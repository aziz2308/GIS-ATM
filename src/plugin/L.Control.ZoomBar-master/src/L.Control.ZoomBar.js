...
L.Control.ZoomBar = L.Control.extend({
  ...
  onAdd: function (map) {
    ...
    this._zoomStartButton = this._createButton(
      "",
    ...
    if (this._isSmallScreen() === false) {
      this._zoomToAreaButton = this._createButton(
        "",
        "Zoom to Area",
  ...
  _startElastic: function (e) {
    ...
    this._zoomRect = new L.rectangle(
      this._zoomBounds,
      this._zoomRectStyle
    ).addTo(this._map);
  ...
  _stopElastic: function (e) {
    ...
    // return the map to its initial state: no crosshairs, re-allow dragging.
    //
